...
mod imp {
    ...
    fn fmt_u64_to_buf(value: u64, buf: &mut [MaybeUninit<u8>]) -> usize {
        ...
        let digits_written = if value < 10_000 {
            ...
            // ending at buf's end. The buffer has length >= 20.
            // `write_unaligned` is used because `buf` is a byte array with no
            // alignment guarantee for u32.
            unsafe {
                end.sub(4).write_unaligned((bcd_hi_hi | 0x30303030).to_be());
                ...
                // For 17-19 digits, create an overlapping u32 by combining bcd_top
                // with the first byte of bcd_hi_hi, enabling word writes instead
                // of byte-by-byte.
                if leading_zeros == 0 {
                    end.sub(5).write_unaligned((bcd_top | 0x30303030).to_be());
                } else {
                    // Shift bcd_top left to align valid digits, shift bcd_hi_hi right
                    // to get its leading bytes, then OR for overlapping u32.
                    let bcd_top_ascii = bcd_top | 0x30303030;
                    let bcd_hi_hi_ascii = bcd_hi_hi | 0x30303030;
                    let shift = leading_zeros * 8;
                    let overlapping = (bcd_top_ascii << shift) | (bcd_hi_hi_ascii >> (32 - shift));
                    // Write at position that overlaps with bcd_hi_hi (already written above)
                    let write_pos = end.cast::<u8>().sub(20 - leading_zeros);
                    (write_pos as *mut u32).write_unaligned(overlapping.to_be());
                }
            }
            20 - leading_zeros
        };
